...
public class Controller {
    public static void validationData(List<String> data) throws DataFormatException {
        ...
        if (data.get(1).length() >= 2) System.out.println(data.get(1));
            ...
            throw new DataFormatException("Слишком короткое имя !");

        }

        if (data.get(2).length() >= 2) System.out.println(data.get(2));
        else {
            throw new DataFormatException("Слишком короткое отчество !");
        }

        if (data.get(3).length() == 10) System.out.println(data.get(3));
        else {
            throw new DataFormatException("Не корректный ввод даты !");
        }
        ...
        if (data.get(4).length() == 10) {
            ...
            throw new DataFormatException("Номер телефона должен состоять из 10 цифр !");
        }
        ...
        if (data.size() == 6 && data.getFirst().length() >= 2 && data.get(1).length() >= 2
                && data.get(2).length() >= 2 && data.get(3).length() == 10 && data.get(4).length() == 10
